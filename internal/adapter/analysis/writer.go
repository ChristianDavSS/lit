...
func ModifyVariableName(management NodeManagement, node tree.Node, code []byte, filePath string, shouldFix bool, conventionIndex int8) {
	...
	newVarName := refactorVarName(tokens, conventionIndex)

	// get the query, cursor and captures (applying the query to fetch them)
	root := GetAST(code, management.GetLanguage())
	defer root.Close()
	query, cursor, captures := GetCapturesByQueries(management.GetLanguage(),
...
func refactorVarName(tokens []string, conventionIndex int8) string {
	var newName = tokens[0]

	switch conventionIndex {
	case 2:
		newName = ""
