...
func NewJSLanguage(pattern string, shouldFix bool, conventionIndex int8) analysis.NodeManagement {
	return javascript{
		shouldFix:       shouldFix,
		conventionIndex: conventionIndex,
		data: analysis.LanguageData{
			Language: tree.NewLanguage(jsGrammar.Language()),
			Queries:  buildJSQueries(pattern),
		},
	}
}

func buildJSQueries(pattern string) string {
...
func (js javascript) ManageNode(captureNames []string, code []byte, filepath string, node tree.QueryCapture, nodeInfo *domain.FunctionData) {
	...
	case captureNames[node.Index] == "variable.name":
		...
		analysis.ModifyVariableName(js, node.Node, code, filepath, js.shouldFix, js.conventionIndex)
		return
