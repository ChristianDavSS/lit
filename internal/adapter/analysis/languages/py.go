...
func NewPythonLanguage(pattern string, shouldFix bool, conventionIndex int8) analysis.NodeManagement {
	return python{
		shouldFix:       shouldFix,
		conventionIndex: conventionIndex,
		data: analysis.LanguageData{
			Language: tree.NewLanguage(pyGrammar.Language()),
			Queries:  buildPythonQueries(pattern),
		},
	}
}

func buildPythonQueries(pattern string) string {
...
func (p python) ManageNode(captureNames []string, code []byte, filepath string, node tree.QueryCapture, nodeInfo *domain.FunctionData) {
	...
	case captureNames[node.Index] == "variable.name":
		...
		analysis.ModifyVariableName(p, node.Node, code, filepath, p.shouldFix, p.conventionIndex)
		return
