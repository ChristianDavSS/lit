...
func CyclicalComplexity(languageInfo NodeManagement, code []byte, filepath string) []*domain.FunctionData {
	...
	for {
		...
		case query.CaptureNames()[copyOf.Captures[0].Index] == "function":
            ...
            startPos := copyOf.Captures[2].Node.StartPosition()
            domainStartPos := domain.Point{Row: startPos.Row, Column: startPos.Column}
